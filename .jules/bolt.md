...
## 2026-06-15 - [Build Stability and Loop Symmetry in Animation Components]
**Learning:** Duplicate variable declarations (e.g., `SPRITE_SIZE`) and duplicate JSX attributes (e.g., `rel` on anchors) cause critical build failures in Next.js 16 with Turbopack. Furthermore, in distance-based opacity bucketing, thresholds must be mapped correctly where the smallest squared distance corresponds to the highest opacity index to achieve the intended visual "glow" for close nodes.
**Action:** Always audit animation components for redundant definitions during optimization passes. Verify JSX attribute uniqueness and ensure threshold arrays are ordered logically (smallest to largest) before mapping to bucket indices.
