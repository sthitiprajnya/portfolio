...
## 2026-07-16 - [Dynamic Import of Heavy SDKs]
**Learning:** The `@emailjs/browser` SDK was statically imported in multiple files, causing it to be included in the initial Next.js JavaScript bundle, thereby increasing page load times despite only being used during explicit form submission.
**Action:** Heavy third-party SDKs like `@emailjs/browser` should be dynamically imported (e.g., `const emailjs = (await import('@emailjs/browser')).default;` inside event handlers) rather than statically imported at the module level. This significantly reduces the initial bundle size and avoids blocking the main thread during initial load.
