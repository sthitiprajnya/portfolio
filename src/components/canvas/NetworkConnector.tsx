...
// ⚡ Optimization: Using dynamic thresholds based on MAX_DISTANCE for better maintainability while keeping them hoisted.
// T1 is the smallest distance (1/6), T5 is the largest (5/6).
const T1_SQ = Math.pow(MAX_DISTANCE * (1 / 6), 2);
const T2_SQ = Math.pow(MAX_DISTANCE * (2 / 6), 2);
const T3_SQ = Math.pow(MAX_DISTANCE * (3 / 6), 2);
const T4_SQ = Math.pow(MAX_DISTANCE * (4 / 6), 2);
const T5_SQ = Math.pow(MAX_DISTANCE * (5 / 6), 2);
      ...
      for (let i = 0; i < numNodes; i++) {
        ...
        for (let j = i + 1; j < numNodes; j++) {
          ...
          if (d2 < MAX_DISTANCE_SQ) {
            ...
            let bIdx = 0;
            if (d2 < T1_SQ) bIdx = 5;
            else if (d2 < T2_SQ) bIdx = 4;
            else if (d2 < T3_SQ) bIdx = 3;
            else if (d2 < T4_SQ) bIdx = 2;
            else if (d2 < T5_SQ) bIdx = 1;
