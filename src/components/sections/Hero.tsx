    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[900px] px-6 flex flex-col items-center text-center flex-1 justify-center">
        ...
        >
          ...
          <CyberButton color="amber" onClick={() => { triggerRef.current = null; setShowMethodology(true); }} aria-haspopup="dialog" aria-controls="methodology-modal" aria-expanded={showMethodology}>VIEW_METHODOLOGY</CyberButton>
          <CyberButton color="green" onClick={() => scrollTo('contact')}>CONTACT_ME</CyberButton>

          <div className="flex space-x-4 pt-4 sm:pt-0 sm:ml-4">
            <a href={PERSONAL.linkedin} target="_blank" rel="noopener noreferrer" aria-label="Visit LinkedIn profile" title="Visit LinkedIn profile"
               className="p-3 min-w-[44px] min-h-[44px] flex items-center justify-center border border-border rounded-card text-text-secondary hover:text-cyan hover:border-cyan hover:shadow-[var(--glow-cyan-sm)] transition-all outline-none focus-visible:ring-2 focus-visible:ring-cyan focus-visible:ring-offset-2 focus-visible:ring-offset-black">
              <span className="sr-only">LinkedIn</span>
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
              </svg>
            </a>
            <a href={PERSONAL.github} target="_blank" rel="noopener noreferrer" aria-label="Visit GitHub profile" title="Visit GitHub profile"
               className="p-3 min-w-[44px] min-h-[44px] flex items-center justify-center border border-border rounded-card text-text-secondary hover:text-cyan hover:border-cyan hover:shadow-[var(--glow-cyan-sm)] transition-all outline-none focus-visible:ring-2 focus-visible:ring-cyan focus-visible:ring-offset-2 focus-visible:ring-offset-black">
              ...
              <div className="space-y-4 font-mono">
                ...
                <div className="mt-8 pt-6 border-t border-border">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <button type="button" onClick={() => { setShowMethodology(false); scrollTo('projects'); }} className="px-3 py-1.5 border border-cyan/30 text-cyan text-xs rounded-card hover:bg-cyan/10 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-cyan focus-visible:ring-offset-2 focus-visible:ring-offset-black" aria-label="Scroll to DLP Pipeline project" title="Scroll to DLP Pipeline project">DLP Pipeline</button>
                    <button type="button" onClick={() => { setShowMethodology(false); scrollTo('projects'); }} className="px-3 py-1.5 border border-cyan/30 text-cyan text-xs rounded-card hover:bg-cyan/10 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-cyan focus-visible:ring-offset-2 focus-visible:ring-offset-black" aria-label="Scroll to GCP Hardening project" title="Scroll to GCP Hardening project">GCP Hardening</button>
                    <button type="button" onClick={() => { setShowMethodology(false); scrollTo('projects'); }} className="px-3 py-1.5 border border-cyan/30 text-cyan text-xs rounded-card hover:bg-cyan/10 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-cyan focus-visible:ring-offset-2 focus-visible:ring-offset-black" aria-label="Scroll to MQTT Attack Chain project" title="Scroll to MQTT Attack Chain project">MQTT Attack Chain</button>
                  </div>
                </div>
              </div>
            </motion.div>
