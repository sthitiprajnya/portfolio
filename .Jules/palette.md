...
## 2026-07-10 - Character Counter Accessibility and Robustness
**Learning:** When building visual character counters for text inputs (especially those with `maxLength`), using `aria-live` on the counter creates significant screen reader spam by announcing updates on every keystroke. Additionally, dynamically constructing `aria-describedby` with conditional targets using `clsx` can result in empty string targets if no IDs are present, causing screen reader parsing errors. Attempting to read `length` on uninitialized input states can cause runtime crashes.
**Action:** Always avoid `aria-live` for keystroke-level character counters; rely instead on visually linking and programmatically binding them using `aria-describedby`. Append `|| undefined` (e.g., `clsx(...) || undefined`) to dynamically constructed ARIA attributes to completely omit them from the DOM when empty. Safely access lengths using optional chaining with fallbacks (e.g., `value?.length || 0`).
