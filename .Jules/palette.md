...
## 2026-07-03 - aria-describedby Empty String Fix and Character Counters
**Learning:** When dynamically setting ARIA attributes like `aria-describedby` using conditionals or libraries like `clsx` in React, returning an empty string (when conditions are not met) creates an accessibility error. Additionally, character counters for inputs with `maxLength` should be visually present and semantically linked using `aria-describedby`.
**Action:** Always append `|| undefined` (e.g., `aria-describedby={clsx(...) || undefined}`) to ensure the attribute is completely omitted from the DOM when no valid ID is present, preventing screen reader errors from parsing empty strings or 'false' as targets. And always provide a character counter linked with `aria-describedby` for inputs with `maxLength`.
