...
## 2026-07-10 - Dynamic Import of Heavy SDKs
**Learning:** Heavy third-party SDKs like `@emailjs/browser` block the main thread and increase bundle size if statically imported at the module level.
**Action:** Use dynamic imports (`const emailjs = (await import('@emailjs/browser')).default;`) inside event handlers (like form submissions) to defer loading and improve initial page load performance.
