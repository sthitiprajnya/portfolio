...
export function ResumePanel() {
  ...
  const handleCopy = async (text: string, setStatus: (s: boolean) => void) => {
    try {
                ...
                <span className="text-[0.6rem] text-text-muted font-mono text-center md:text-left">
                  DOC_ID: <span className="relative inline-block">
                    <span
                      ...
                      onClick={() => handleCopy('SB-RESUME-2025-v3', setCopiedDocId)}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          handleCopy('SB-RESUME-2025-v3', setCopiedDocId);
                        }
                      }}
                    >
                      SB-RESUME-2025-v3
                    ...
                    <AnimatePresence>
                      {copiedDocId && (
                        <motion.span
                          ...
                          role="status"
                          aria-live="polite"
                          className="absolute bottom-full left-1/2 mb-2 px-2 py-1 bg-cyan text-black font-mono text-[0.5rem] rounded-card font-bold shadow-[var(--glow-cyan-sm)] z-50 pointer-events-none whitespace-nowrap"
                        >
                          COPIED!
                        </motion.span>
                      )}
                  ...
                  <span className="hidden md:inline"> · </span>SHA256: <span className="relative inline-block">
                    <span
                      ...
                      onClick={() => handleCopy(RESUME_SHA256, setCopiedHash)}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          handleCopy(RESUME_SHA256, setCopiedHash);
                        }
                      }}
                    >
                      {RESUME_SHA256}
