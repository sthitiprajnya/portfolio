...
## 2026-06-05 - [Pre-parsing RGBA Constants at Module Level]
**Learning:** In 60fps animation loops, such as the `draw()` loop in `Sentinel.tsx`, executing Regular Expressions and calling `parseInt` inside the hot loop to parse string constants (like `'rgba(255,255,255,1)'`) into numeric objects causes measurable CPU overhead and creates unnecessary Garbage Collection pressure, contributing to jank.
**Action:** When a React component needs to rapidly and repeatedly parse static string configurations, the parsed data structures should be hoisted to the module level and pre-calculated only once during initialization.
