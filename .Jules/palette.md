...
## 2026-07-09 - Character Counters and aria-live
**Learning:** When adding visual character counters to text inputs or textareas, avoid using `aria-live` on the counter element. Announcing updates on every keystroke creates significant screen reader spam. Instead, rely on linking the counter visually and programmatically using `aria-describedby`.
**Action:** When implementing inputs with `maxLength`, provide a visually linked character counter via `aria-describedby` and omit `aria-live`.
