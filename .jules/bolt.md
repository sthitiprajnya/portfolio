...
## 2024-05-18 - React State vs Direct DOM Manipulation in High-Frequency Animations
**Learning:** Using `useState` inside a `setInterval` loop that runs every 120ms or 300ms causes severe React reconciliation overhead and triggers unnecessary DOM re-renders, especially for simple class toggles in components like `AsciiAvatar.tsx`.
**Action:** When implementing high-frequency UI updates (like terminal scan lines or typing effects), bypass React state. Use `useRef` to store DOM elements (`HTMLDivElement[]`) and manipulate their `classList` directly. Crucially, also store the animation's internal tracking state (like the current line index) inside a `useRef` so the animation state is preserved across `useEffect` re-runs (e.g., when the component is scrolled out of view and back in).
