...
## 2026-07-08 - Lazy-load EmailJS SDK
**Learning:** Heavy third-party SDKs like @emailjs/browser add significant overhead to the initial Next.js JavaScript bundle size. This blocks the main thread during the initial page load, causing performance issues.
**Action:** Always dynamically import heavy SDKs (e.g., `const emailjs = (await import('@emailjs/browser')).default;`) inside event handlers (like form submission) rather than statically importing them at the module level.
