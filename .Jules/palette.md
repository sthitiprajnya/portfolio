...
## 2024-07-03 - Character Counter Consistency in Forms
**Learning:** Found an inconsistency where `FloatingTextarea` had a character limit counter but `FloatingInput` did not, despite both supporting the `maxLength` prop. Adding a visual counter to inputs with a hard limit provides immediate feedback and prevents user frustration.
**Action:** When creating form inputs that accept a `maxLength` restriction, always provide a visual character counter and ensure it is linked to the input via `aria-describedby`.
