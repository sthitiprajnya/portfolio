...
## 2026-05-31 - [True Lazy Loading of Chart.js]
**Learning:** `ChartJS.register()` at the module level completely defeats `next/dynamic` lazy loading for `react-chartjs-2`, pulling the ~50KB (gzipped) library into the main bundle. Additionally, removing `ChartData` and `ChartOptions` types causes linting errors with `@typescript-eslint/no-explicit-any`.
**Action:** Move `ChartJS.register()` inside a dedicated, dynamically imported wrapper component (`RadarChart.tsx`). Maintain type safety using `import type { ChartData, ChartOptions } from 'chart.js'` and spread original options (e.g., `animation.duration`) to preserve UI behavior.
