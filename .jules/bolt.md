...
## 2025-05-22 - [Batching Canvas Draw Calls and Hoisting for Animation Loops]
**Learning:** In high-frequency (60fps) Canvas 2D loops, performing thousands of `fillStyle` assignments and string template allocations (like `rgba(...)`) every frame causes significant CPU overhead and garbage collection (GC) pauses. By pre-calculating color strings and batching `fillText` calls by color (iterating trail levels then columns), state changes can be reduced by ~98%. Using `TypedArrays` (`Float32Array`, `Uint8Array`) and bitmasks further improves memory efficiency and lookup performance.
**Action:** Always hoist static data and pre-calculate dynamic strings in animation loops. Batch draw calls by shared state (color, line width) to minimize context changes. Use bitmasks for O(1) flag checks in hot loops.
