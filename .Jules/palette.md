...
## 2026-06-24 - Screen Reader Spam from Animated Counters
**Learning:** Components that animate numbers dynamically (like `react-countup` changing from 0 to 100) are announced repeatedly by screen readers on every DOM update, creating a chaotic "number spam" experience.
**Action:** Always wrap the animating number element in `aria-hidden="true"` and provide the final static value alongside it in a `.sr-only` (visually hidden) span.
