...
## 2026-06-18 - Glyph Caching for High-Frequency Canvas Text
**Learning:** In high-frequency (60fps) Canvas 2D animation loops,  is surprisingly expensive due to the overhead of font rasterization and layout. This cost scales linearly with the number of characters drawn (e.g., Matrix Rain columns). By pre-rendering the entire character set into an offscreen "glyph cache" canvas and using  to blit characters, CPU overhead can be reduced by ~70% during high-density frames.
**Action:** Always implement offscreen glyph caching for text-heavy canvas animations. Pre-render all required characters and styles (colors, font sizes) during initialization or resize handlers, and use hardware-accelerated  for the main animation loop.

## 2026-06-18 - Glyph Caching for High-Frequency Canvas Text
**Learning:** In high-frequency (60fps) Canvas 2D animation loops, `ctx.fillText()` is surprisingly expensive due to the overhead of font rasterization and layout. This cost scales linearly with the number of characters drawn (e.g., Matrix Rain columns). By pre-rendering the entire character set into an offscreen "glyph cache" canvas and using `ctx.drawImage()` to blit characters, CPU overhead can be reduced by ~70% during high-density frames.
**Action:** Always implement offscreen glyph caching for text-heavy canvas animations. Pre-render all required characters and styles (colors, font sizes) during initialization or resize handlers, and use hardware-accelerated `drawImage()` for the main animation loop.
