...
## 2026-08-20 - [Glyph Caching for Canvas Text]
**Learning:** In high-frequency (60fps) Canvas 2D loops,  is surprisingly expensive as it requires the browser to perform text layout, shaping, and rasterization on every frame. By pre-rendering the character set into an offscreen canvas "glyph cache" across all required colors and styles, we can replace these calls with hardware-accelerated `ctx.drawImage()` blits. This significantly reduces CPU overhead and improves frame stability, especially for dense text-based animations like Matrix Rain.
**Action:** For components rendering many characters per frame, implement a glyph cache during initialization or resize. Use `drawImage` to blit characters from the cache in the hot loop.

## 2026-08-20 - [Glyph Caching for Canvas Text]
**Learning:** In high-frequency (60fps) Canvas 2D loops, `ctx.fillText()` is surprisingly expensive as it requires the browser to perform text layout, shaping, and rasterization on every frame. By pre-rendering the character set into an offscreen canvas "glyph cache" across all required colors and styles, we can replace these calls with hardware-accelerated `ctx.drawImage()` blits. This significantly reduces CPU overhead and improves frame stability, especially for dense text-based animations like Matrix Rain.
**Action:** For components rendering many characters per frame, implement a glyph cache during initialization or resize. Use `drawImage` to blit characters from the cache in the hot loop.
