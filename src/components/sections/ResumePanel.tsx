...
export function ResumePanel() {
  ...
  const handleCopy = async (text: string, type: 'doc' | 'hash' | 'link') => {
    try {
            ...
            <div className="overflow-hidden border border-[var(--glass-border)] hover:shadow-[var(--glow-amber-sm)] glass-heavy rounded-card relative" data-orb-target="true">
              ...
              <div ref={iframeRef} className="w-full bg-[rgba(0,0,0,0.4)] relative z-10 min-h-[600px] flex items-center justify-center">
                {inView ? (
                  <iframe
                    ...
                    title="Resume PDF"
                    // Security: Use the most restrictive sandbox possible.
                    // Omitting 'allow-same-origin' treats the iframe as a unique origin,
                    // isolating it from the main site's sensitive data (cookies, storage).
                    sandbox=""
              ...
              <div className="px-6 py-4 border-t border-[var(--glass-border)] flex flex-col md:flex-row justify-between items-center bg-[rgba(0,0,0,0.6)] gap-4 relative z-10">
                <span className="text-[0.6rem] text-text-muted font-mono text-center md:text-left">
                  DOC_ID: <span
                    className="redacted group/doc relative"
                    tabIndex={0}
                    role="button"
                    ...
                    <AnimatePresence>
                      {copiedDocId && (
                        <motion.span
                          ...
                          className="absolute bottom-full left-1/2 mb-4 px-2 py-1 bg-amber text-black font-mono text-[0.6rem] rounded-card font-bold shadow-[var(--glow-amber-sm)] z-50 pointer-events-none whitespace-nowrap"
                        >
                          COPIED!
                        </motion.span>
                      )}
                  ...
                  <span className="hidden md:inline"> · </span>SHA256: <span
                    className="redacted group/hash relative"
                    tabIndex={0}
                    role="button"
