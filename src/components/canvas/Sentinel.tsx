...
export default function Sentinel() {
  ...
  useEffect(() => {
    ...
    let lastRenderedR = -1, lastRenderedG = -1, lastRenderedB = -1, lastRenderedA = -1;
    ...
    const updateSprite = (
      ...
      if (
        Math.abs(fCR - lastRenderedR) < 1 &&
        Math.abs(fCG - lastRenderedG) < 1 &&
        Math.abs(fCB - lastRenderedB) < 1 &&
        Math.abs(fCA - lastRenderedA) < 0.01
      ) return;
    ...
    const draw = () => {
      ...
      for (let i = 0; i < trailLen; i++) {
        ...
        const trailYOffset = ty + (cy - ty) * 0.5; // Adjusted for better follow effect
